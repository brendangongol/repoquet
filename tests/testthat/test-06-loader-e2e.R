...
test_that("an unconfigured per-partition budget remains bounded below the whole-file estimate", {
  ...
  expect_true(any(grepl("falling back to memory-capped chunking", r$output, fixed = TRUE)))
  rows_per_year <- sapply(2020:2023, function(yr) {
    files <- list.files(file.path(fx$pq, "REG_Tiered3", sprintf("year=%d", yr)), full.names = TRUE)
    sum(vapply(files, function(f) nrow(arrow::read_parquet(f)), integer(1)))
  })
...
test_that("FAIL mode uses an isolated worker for a multi-year CSV", {
  ...
  worker_log <- readLines(fx$log, warn = FALSE)
  expect_true(any(grepl("Worker direct read complete for fail_route.csv", worker_log, fixed = TRUE)))
  expect_true(any(grepl("[PARQUET WRITE] fail_route.csv: partition year=2022, piece 1", worker_log, fixed = TRUE)))
